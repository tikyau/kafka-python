...
class DefaultRecordBatchBuilder(DefaultRecordBase, ABCRecordBatchBuilder):
    ...
    def append(self, offset, timestamp, key, value, headers):
        ...
        required_size = message_len + len(message_len_enc)
        # Check if we can write this message
        if not (first_message or
                required_size + len(self._buffer) <= self._batch_size):
            return None, 0

        self._buffer += message_len_enc + message_buffer.getvalue()
